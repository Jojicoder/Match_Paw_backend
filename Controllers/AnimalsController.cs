...
public class AnimalsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAnimals()
    {
        ...
        var sql = @"SELECT animal_id, name, species, breed, age, sex, intake_date,
                           adoption_status, health_status, notes, photo_url, created_at
    ...
    public async Task<IActionResult> CreateAnimal([FromBody] AnimalRequest req)
    {
        ...
        var sql = @"INSERT INTO animals (name, species, breed, age, sex, intake_date, adoption_status, health_status, notes, photo_url)
                    VALUES (@name, @species, @breed, @age, @sex, @intakeDate, @adoptionStatus, @healthStatus, @notes, @photoUrl);
        ...
        command.Parameters.AddWithValue("@photoUrl", (object?)req.PhotoUrl ?? DBNull.Value);
    ...
    public async Task<IActionResult> UpdateAnimal(int id, [FromBody] AnimalRequest req)
    {
        ...
        var sql = @"UPDATE animals SET
                        ...
                        health_status = @healthStatus, notes = @notes, photo_url = @photoUrl
                    WHERE animal_id = @id";
    ...
    [HttpPatch("{id}/photo")]
    public async Task<IActionResult> UpdateAnimalPhoto(int id, [FromBody] UpdatePhotoRequest req)
    {
        await using var connection = new MySqlConnection(_configuration.GetConnectionString("DefaultConnection"));
        await connection.OpenAsync();

        var sql = "UPDATE animals SET photo_url = @photoUrl WHERE animal_id = @id";
        await using var command = new MySqlCommand(sql, connection);
        command.Parameters.AddWithValue("@id", id);
        command.Parameters.AddWithValue("@photoUrl", (object?)req.PhotoUrl ?? DBNull.Value);
